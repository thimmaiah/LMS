...
public class CourseResource {
     ...
     *         body the new courseDTO, or with status {@code 400 (Bad Request)} if
     *         the course has already an ID.
     ...
     *         the updated courseDTO, or with status {@code 400 (Bad Request)} if
     *         the courseDTO is not valid, or with status
     *         {@code 500 (Internal Server Error)} if the courseDTO couldn't be
     *         updated.
     * @throws URISyntaxException if the Location URI syntax is incorrect.
     */
    @PutMapping("/courses")
     ...
     * @return the {@link ResponseEntity} with status {@code 200 (OK)} and with body
     *         the courseDTO, or with status {@code 404 (Not Found)}.
     */
    ...
    /**
     * {@code SEARCH  /_search/courses?query=:query} : search for the course
    ...
    /**
     * {@code POST /courses/{id}/addSme/{login}} : Adds the SME to the existing
     * course.
     *
     * @param id    the id of the course to update.
     * @param login the login of the SME to be added to the course .
     * @return the {@link ResponseEntity} with status {@code 200 (OK)} and with body
     *         the updated courseDTO, or with status {@code 400 (Bad Request)} if
     *         the courseDTO is not valid, or with status
     *         {@code 500 (Internal Server Error)} if the courseDTO couldn't be
    ...
    public ResponseEntity<CourseDTO> addSmeToCourse(@PathVariable Long id, @PathVariable String login)
    ...
    /**
     * {@code POST /courses/{id}/register/{login}} : Adds the SME to the existing
     * course.
     *
     * @param id    the id of the course to update.
     * @param login the login of the user to be registered to the course .
     * @return the {@link ResponseEntity} with status {@code 200 (OK)} and with body
     *         the updated courseDTO, or with status {@code 400 (Bad Request)} if
     *         the courseDTO is not valid, or with status
     *         {@code 500 (Internal Server Error)} if the courseDTO couldn't be
     ...
     * @throws URISyntaxException if the Location URI syntax is incorrect.
     */
    @PostMapping("/courses/{id}/register/{login}")
    @PreAuthorize("hasAuthority('ROLE_USER')")
    public ResponseEntity<CourseDTO> register(@PathVariable Long id, @PathVariable String login)
            throws URISyntaxException {
        log.debug("REST request to register user {} to Course {}", login, id);
        Optional<CourseDTO> course = courseService.findOne(id);
        course.ifPresent(courseDto -> {
            Optional<com.learnnow.domain.User> user = userService.findOne(login);
            user.ifPresent(reg -> {
                attendenceService.register(courseDto, new UserDTO(reg));
            });
        });
    ...
    /**
     * {@code POST /courses/{id}/deregister/{login}} : Adds the SME to the existing
     * course.
     *
     * @param id    the id of the course to update.
     * @param login the login of the user to be deregistered from the course .
     * @return the {@link ResponseEntity} with status {@code 200 (OK)} and with body
     *         the updated courseDTO, or with status {@code 400 (Bad Request)} if
     *         the courseDTO is not valid, or with status
     *         {@code 500 (Internal Server Error)} if the courseDTO couldn't be
     ...
     * @throws URISyntaxException if the Location URI syntax is incorrect.
     */
    @PostMapping("/courses/{id}/deregister/{login}")
    @PreAuthorize("hasAuthority('ROLE_USER')")
    public ResponseEntity<CourseDTO> deregister(@PathVariable Long id, @PathVariable String login)
            throws URISyntaxException {
        log.debug("REST request to deregister user {} to Course {}", login, id);
        Optional<CourseDTO> course = courseService.findOne(id);
        course.ifPresent(courseDto -> {
            Optional<com.learnnow.domain.User> user = userService.findOne(login);
            user.ifPresent(reg -> {
                attendenceService.deregister(courseDto, new UserDTO(reg));
            });
        });
        return ResponseUtil.wrapOrNotFound(course);
    }
